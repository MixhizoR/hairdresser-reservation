...
- [x] Task: Remove 'Completed' filter and status update buttons from `AdminPage.jsx`. 323e9cb
- [x] Task: Remove `completed` status translation and color coding from `App.jsx`, `HomePage.jsx`, and `StatusPage.jsx`. 323e9cb
- [x] Task: Update frontend tests (Vitest) to ensure the `completed` status is no longer used or expected. 805625a
