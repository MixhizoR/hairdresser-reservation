...
describe("Completed Status Removal Verification", () => {
  ...
  describe("No API endpoint returns completed status", () => {
    ...
    it("POST /api/appointments should create with pending status, not completed", async () => {
      dbService.findServiceByName.mockResolvedValue({
        id: "svc-1",
        name: "Saç Kesimi",
        isActive: true,
        duration: 30
      ...
      // CRITICAL FIX: Skip Sunday (day 0) to comply with system operational hours
      if (futureDate.getDay() === 0) {
        futureDate.setDate(futureDate.getDate() + 1); // Move to Monday
      }

      futureDate.setHours(10, 0, 0, 0); // 10:00 AM is safely within 08:00-21:00
